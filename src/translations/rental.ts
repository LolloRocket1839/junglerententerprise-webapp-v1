export const rentalTranslations = {
  en: {
    // Status
    available: "Available",
    notAvailable: "Not available",
    
    // Property Info
    marketPrice: "Market price",
    discountedPrice: "Discounted price",
    rooms: "Rooms",
    bathrooms: "Bathrooms",
    surface: "Surface",
    viewDetails: "View details",
    details: "Details",
    
    // Property Features
    distanceToUniversity: "Distance to university",
    savingsPercentage: "Savings",
    
    // Unified Property Labels
    invest: "Invest",
    book: "Book", 
    rent: "Rent",
    collected: "Collected",
    target: "Target",
    priceOnRequest: "Price on request",
    perNight: "per night",
    perMonth: "per month",
    
    // Room and Swap Cards
    contact: "Contact",
    postedBy: "Posted by",
    room: "Room",
    price: "Price",
    lookingFor: "Looking for",
    description: "Description",
    furniture: "Furniture",
    amenities: "Amenities", 
    reviews: "reviews",
    perMonth2: "per month",
    
    // Admin interface
    edit: "Edit",
    delete: "Delete",
    addProperty: "Add Property"
  },
  
  it: {
    // Status
    available: "Disponibile",
    notAvailable: "Non disponibile",
    
    // Property Info
    marketPrice: "Prezzo di mercato",
    discountedPrice: "Prezzo scontato",
    rooms: "Stanze",
    bathrooms: "Bagni", 
    surface: "Superficie",
    viewDetails: "Visualizza dettagli",
    details: "Dettagli",
    
    // Property Features
    distanceToUniversity: "Distanza dall'università",
    savingsPercentage: "Risparmio",
    
    // Unified Property Labels
    invest: "Investi",
    book: "Prenota",
    rent: "Affitta",
    collected: "Raccolti",
    target: "Obiettivo",
    priceOnRequest: "Prezzo su richiesta",
    perNight: "per notte",
    perMonth: "per mese",
    
    // Room and Swap Cards
    contact: "Contatta",
    postedBy: "Pubblicato da",
    room: "Camera",
    price: "Prezzo",
    lookingFor: "Cerco",
    description: "Descrizione",
    furniture: "Arredamento",
    amenities: "Servizi",
    reviews: "recensioni",
    perMonth2: "al mese",
    
    // Admin interface
    edit: "Modifica",
    delete: "Elimina",
    addProperty: "Aggiungi Proprietà"
  },
  
  de: {
    // Status
    available: "Verfügbar",
    notAvailable: "Nicht verfügbar",
    
    // Property Info
    marketPrice: "Marktpreis",
    discountedPrice: "Ermäßigter Preis",
    rooms: "Zimmer",
    bathrooms: "Badezimmer",
    surface: "Fläche",
    viewDetails: "Details anzeigen",
    details: "Details",
    
    // Property Features
    distanceToUniversity: "Entfernung zur Universität",
    savingsPercentage: "Einsparungen",
    
    // Unified Property Labels
    invest: "Investieren",
    book: "Buchen",
    rent: "Mieten", 
    collected: "Gesammelt",
    target: "Ziel",
    priceOnRequest: "Preis auf Anfrage",
    perNight: "pro Nacht",
    perMonth: "pro Monat",
    
    // Room and Swap Cards
    contact: "Kontakt",
    postedBy: "Veröffentlicht von",
    room: "Zimmer",
    price: "Preis",
    lookingFor: "Suche nach",
    description: "Beschreibung",
    furniture: "Möbel",
    amenities: "Ausstattung",
    reviews: "Bewertungen",
    perMonth2: "pro Monat",
    
    // Admin interface
    edit: "Bearbeiten",
    delete: "Löschen",
    addProperty: "Immobilie hinzufügen"
  },
  
  ro: {
    // Status
    available: "Disponibil",
    notAvailable: "Nu este disponibil",
    
    // Property Info
    marketPrice: "Preț de piață",
    discountedPrice: "Preț redus",
    rooms: "Camere",
    bathrooms: "Băi",
    surface: "Suprafață",
    viewDetails: "Vezi detalii",
    details: "Detalii",
    
    // Property Features
    distanceToUniversity: "Distanța până la universitate",
    savingsPercentage: "Economii",
    
    // Unified Property Labels
    invest: "Investește",
    book: "Rezervă",
    rent: "Închiriază",
    collected: "Colectat",
    target: "Țintă",
    priceOnRequest: "Preț la cerere",
    perNight: "pe noapte",
    perMonth: "pe lună",
    
    // Room and Swap Cards
    contact: "Contact",
    postedBy: "Publicat de",
    room: "Cameră",
    price: "Preț",
    lookingFor: "Caut",
    description: "Descriere",
    furniture: "Mobilier",
    amenities: "Facilități",
    reviews: "recenzii",
    perMonth2: "pe lună",
    
    // Admin interface
    edit: "Editează",
    delete: "Șterge",
    addProperty: "Adaugă Proprietate"
  },
  
  es: {
    // Status
    available: "Disponible",
    notAvailable: "No disponible",
    
    // Property Info
    marketPrice: "Precio de mercado",
    discountedPrice: "Precio descontado",
    rooms: "Habitaciones",
    bathrooms: "Baños",
    surface: "Superficie",
    viewDetails: "Ver detalles",
    details: "Detalles",
    
    // Property Features
    distanceToUniversity: "Distancia a la universidad",
    savingsPercentage: "Ahorros",
    
    // Unified Property Labels
    invest: "Invertir",
    book: "Reservar",
    rent: "Alquilar",
    collected: "Recaudado",
    target: "Objetivo",
    priceOnRequest: "Precio bajo consulta",
    perNight: "por noche",
    perMonth: "por mes",
    
    // Room and Swap Cards
    contact: "Contactar",
    postedBy: "Publicado por",
    room: "Habitación",
    price: "Precio",
    lookingFor: "Busco",
    description: "Descripción",
    furniture: "Mobiliario",
    amenities: "Servicios",
    reviews: "reseñas",
    perMonth2: "al mes",
    
    // Admin interface
    edit: "Editar",
    delete: "Eliminar",
    addProperty: "Añadir Propiedad"
  },
  
  fr: {
    // Status
    available: "Disponible",
    notAvailable: "Non disponible",
    
    // Property Info
    marketPrice: "Prix du marché",
    discountedPrice: "Prix réduit",
    rooms: "Chambres",
    bathrooms: "Salles de bain",
    surface: "Surface",
    viewDetails: "Voir les détails",
    details: "Détails",
    
    // Property Features
    distanceToUniversity: "Distance à l'université",
    savingsPercentage: "Économies",
    
    // Unified Property Labels
    invest: "Investir",
    book: "Réserver",
    rent: "Louer",
    collected: "Collecté",
    target: "Objectif",
    priceOnRequest: "Prix sur demande",
    perNight: "par nuit",
    perMonth: "par mois",
    
    // Room and Swap Cards
    contact: "Contact",
    postedBy: "Publié par",
    room: "Chambre",
    price: "Prix",
    lookingFor: "Je cherche",
    description: "Description",
    furniture: "Mobilier",
    amenities: "Équipements",
    reviews: "avis",
    perMonth2: "par mois",
    
    // Admin interface
    edit: "Modifier",
    delete: "Supprimer",
    addProperty: "Ajouter une Propriété"
  }
};